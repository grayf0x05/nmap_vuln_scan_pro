...
# ====================== PRIVILEGI ======================
def ensure_root():
    """Richiede privilegi root (POSIX). Su Windows stampa avviso e termina (non supportato)."""
    system_platform = platform.system()
    if system_platform == "Windows":
        print("[!] Windows non è supportato da questo script (path script NSE e privilegi). Usa Linux o macOS.")
        sys.exit(1)
    try:
        if os.geteuid() != 0:
        ...
        print("[i] Avviso: impossibile verificare privilegi (piattaforma non POSIX).")

# ====================== RILEVAMENTO PERCORSI SCRIPT NMAP ======================
def detect_nmap_scripts_base():
    """Prova a individuare la directory 'scripts' di Nmap su Linux/macOS."""
    if shutil.which("nmap"):
        candidates = [
            "/usr/share/nmap/scripts",
            "/usr/local/share/nmap/scripts",
            "/opt/homebrew/share/nmap/scripts",      # macOS Apple Silicon (Homebrew)
            "/usr/local/opt/nmap/share/nmap/scripts" # macOS Homebrew (alt)
        ]
        for c in candidates:
            if os.path.isdir(c):
                return c
...
SCRIPTS_BASE = detect_nmap_scripts_base()
if not SCRIPTS_BASE:
    print("[!] Directory script Nmap non trovata. Assicurati che Nmap sia installato e riprova.")
    sys.exit(1)

# Percorsi delle repo NSE da aggiornare/clonare
...
def detect_pkg_manager():
    for pm in ["apt", "dnf", "yum", "pacman", "brew"]:
        if shutil.which(pm):
            return pm
    return None
...
def install_dependencies(tools):
    ...
    try:
        if pm == "apt":
            ...
            for tool in tools:
                subprocess.run([pm, "install", tool], check=True)
        else:
            print(f"[!] Sistema operativo/PM non supportato. Installa manualmente: {', '.join(tools)}")
            sys.exit(1)
...
def update_nmap_scripts():
    ...
    # Verifica che gli script siano registrati (ad es. 'vulners')
    try:
        subprocess.run(["nmap", "--script-help", "vulners"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
    except subprocess.CalledProcessError:
        print("[!] Avviso: Nmap non trova lo script 'vulners'. Verifica i percorsi e i permessi in:", SCRIPTS_BASE)

# ====================== COSTRUZIONE COMANDO NMAP ======================
def build_nmap_command(target, profile, output_xml, vm_safe=True, pn=False, no_dns=False, host_timeout="5m"):
    # Usare nomi script robusti (Nmap risolve per nome file nella script db)
    scripts = ["vuln", "vulners", "vulscan"]
    extra_flags = ["--stats-every", "5s", "--host-timeout", host_timeout]

    if profile == "1":
        extra_flags += ["-T3", "--top-ports", "100"]
    elif profile == "2":
        scripts += ["ssl-cert", "ssl-enum-ciphers", "ftp-anon",
                    "smb2-security-mode", "smb2-capabilities", "smb2-time"]
    ...
    if vm_safe:
        # Impostazioni più prudenti su reti sensibili/VM
        extra_flags += ["--min-rate", "10", "--max-rate", "200", "-T2", "--defeat-rst-ratelimit", "--max-retries", "1"]

    if pn:
        extra_flags.append("-Pn")
    if no_dns:
        extra_flags.append("-n")
...
# CVSS più robusto: supporta 'CVSS:10', 'CVSS 10', 'CVSS v2 7.5', 'CVSS v3.1 9.8'
CVSS_REGEX = re.compile(
    r"CVSS(?:\s*v[23](?:\.\d)?)?[:\s]*([0-9]+(?:\.[0-9])?)",
    re.IGNORECASE
)
...
def generate_html_report_from_xml(xml_file, output_file="report.html"):
    ...
    for info in host_data.values():
        for cve in info['cves']:
            sev_key = cve.get('severity', 'Info')
            severity_count[sev_key] = severity_count.get(sev_key, 0) + 1
    ...
    <table aria-label="Riassunto scansione">
    ...
    <table aria-label="Distribuzione severità">
    ...
    host_summary = """
    <h2>Dettagli host</h2>
    <table aria-label="Dettagli host">
...
<body>
<noscript><p><strong>Nota:</strong> i grafici richiedono JavaScript. Il report rimane comunque consultabile.</p></noscript>
...
<div class='legend-box'>
  ...
  <p><strong>Severità:</strong>
     ...
     <span class='severity-cell' style='background:#32CD32'>Low</span> (0.0–3.9) •
...
<canvas id="severityChart" aria-label="Grafico a torta severità" role="img"></canvas>
<h2>Vulnerabilità per host (ordinate)</h2>
<canvas id="hostChart" aria-label="Grafico a barre vulnerabilità per host" role="img"></canvas>
<h2>Dettagli CVE</h2>
<div class="severity-filter" aria-label="Filtro severità">
Filtra per severità:
{''.join([f'<label><input type="checkbox" class="sev-filter" value="{sev}" checked> {sev}</label> ' for sev in severity_count])}
</div>
<table aria-label="Tabella dettagli CVE">
...
def save_json_report(xml_file, output_file="report.json", meta=None):
    data = parse_nmap_xml(xml_file)
    if meta:
        data = {"metadata": meta, "results": data}
    with open(output_file, "w", encoding="utf-8") as f:
...
def save_csv_report(xml_file, output_file="report.csv"):
    ...
    with open(output_file, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["host", "hostname", "port", "service", "cve", "severity", "description"])
...
=========================================
        \033[91mNmap Vulnerability Scanner Pro\033[0m{CYAN}{BOLD}
            {GREEN}Created by: grayf0x{RESET}{CYAN}{BOLD}
               {YELLOW}Version: 1.4.0{RESET}{CYAN}{BOLD}
========================================={RESET}
""")
...
def parse_args():
    ...
    # Nuove opzioni
    p.add_argument("--pn", action="store_true", help="Salta discovery host (-Pn)")
    p.add_argument("--no-dns", action="store_true", help="Disabilita risoluzione DNS (-n)")
    p.add_argument("--host-timeout", default="5m", help="Valore per --host-timeout (default: 5m)")
    return p.parse_args()
...
def main():
    ...
    cmd = build_nmap_command(
        target, profile, output_xml,
        vm_safe=args.vm_safe, pn=args.pn, no_dns=args.no_dns,
    ...
    try:
        with open(output_log, 'w', encoding='utf-8') as logf:
            ...
            # Logga stdout e stderr nello stesso file
    ...
    save_json_report(output_xml, output_json, meta=meta)
