...
def parse_nmap_xml(xml_file):
    ...
    for host in root.findall('host'):
        ...
        if not ip:
            continue
        ports = [f"{p.get('portid')}/{p.get('protocol')}" 
...
def generate_html_report_from_xml(xml_file, output_file="report.html"):
    ...
    new Chart(severityCtx, {{
        ...
        data: {{
            ...
            datasets: [{{
                data: {list(severity_count.values())},
                backgroundColor: {list(severity_colors.values())}
            }}]
        }},
        options: {{
            responsive: true,
    ...
    new Chart(hostCtx, {{
        ...
        data: {{
            ...
            datasets: [{{
                ...
                data: {[len(info['cves']) for info in host_data.values()]},
...
# ---------------------- MAIN ----------------------
def main():
    print_banner()  # Mostra il banner all'avvio
    check_dependencies()
    update_vulscan_db()
    update_vulners()
    update_nmap_scripts()
